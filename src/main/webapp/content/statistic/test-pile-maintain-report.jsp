                        ...
                        <div class="portlet-body">
                            <div class="row-fluid">
                                  <div class="span12">
                                            ...
                                            <button  onclick ="$('.statistic-table').tableExport({type:'excel',fileName:'测试桩(井)维护处理报表', excelstyles:['border-bottom', 'border-top', 'border-left', 'border-right']});" class="btn blue input-append"><i class="icon-add"></i>导出Excel</button>
                                          <button  onclick ="$('.statistic-table').tableExport({type:'word',fileName:'测试桩(井)维护处理报表'});" class="btn blue input-append"><i class="icon-add"></i>导出Word</button>
                                   </div>
                                    <!--TODO LIST:此处把table_user改成相关子模块的ID即可-->
                                   <div class="span11" style="height: 28.5em;" id="report-div">
...
<script>
    ...
    $(function () {
        ...
        handleDatePickers();
        TestPileStats.initForMaintainReport();
    })
    $("#searchMarker").click(function(){
        var beginDate = $("#txt_begin_day").val();
        var endDate = $("#txt_end_day").val();
        TestPileStats.initForMaintainReport(beginDate,endDate)
    });
</script>
