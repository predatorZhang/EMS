                            ...
                            <div class="row-fluid">
                                  <div class="span12">
                                       <div class="m-wrap input-append date date-picker"
                                        ...
                                        <button  onclick ="$('.statistic-table').tableExport({type:'excel',fileName:'测试桩状态报表', excelstyles:['border-bottom', 'border-top', 'border-left', 'border-right']});" class="btn blue input-append"  style="margin-bottom: 10px"><i class="icon-add"></i>导出Excel</button>
                                        <button  onclick ="$('.statistic-table').tableExport({type:'word',fileName:'测试桩状态报表'});" class="btn blue input-append" style="margin-bottom: 10px"><i class="icon-add"></i>导出Word</button>
                                  </div>
...
<script>
    ...
    $(function () {
        ...
        handleDatePickers();
        TestPileStats.initForTestPileReport();
    })
    $("#searchMarker").click(function(){
        var year = $("#txt_begin_day").val();
        var ownerComp = $("#belong-comp").val();
        TestPileStats.initForTestPileReport(ownerComp,year)
    });
    $('.date-picker').datepicker({
        format: 'yyyy',
        weekStart: 1,
