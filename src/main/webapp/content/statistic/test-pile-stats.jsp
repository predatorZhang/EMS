                        ...
                        <div class="portlet-body">
                            <div class="row-fluid">
                                  <div class="span12">
                                    ...
                                    <input class="input-append" style="max-width: 6em" placeholder="所属区域" type="text" class="m-wrap" id="area" list="areaList"/>
                                    <datalist id="areaList">
                                        <option value="全部">全部</option>
                                        <option value="运行一所">运行一所</option>
                                        <option value="运行二所">运行二所</option>
                                        <option value="运行三所">运行三所</option>
                                        <option value="运行四所">运行四所</option>
                                        <option value="运行五所">运行五所</option>
                                        <option value="运行六所">运行六所</option>
                                        <option value="运行七所">运行七所</option>
                                    </datalist>
                                    <button type="submit" id="searchMarker" class="btn blue input-append" style="margin-bottom: 10px">查询</button>
                                    <button  onclick ="$('.statistic-table').tableExport({type:'excel',fileName:'测试桩状态统计', excelstyles:['border-bottom', 'border-top', 'border-left', 'border-right']});" class="btn blue input-append" style="margin-bottom: 10px"><i class="icon-add"></i>导出Excel</button>
                                    <button  onclick ="$('.statistic-table').tableExport({type:'word',fileName:'测试桩状态统计'});" class="btn blue input-append" style="margin-bottom: 10px"><i class="icon-add"></i>导出Word</button>
                                        </div>
...
<script>
    ...
    $(function () {
        ...
        handleDatePickers();
        TestPileStats.initForTestPileStatus();
    })
    $("#searchMarker").click(function(){
        ...
        var area = $("#area").val();
        TestPileStats.initForTestPileStatus(beginDate,endDate,ownerComp,area)
    });
</script>
