                        ...
                        <div class="portlet-body">
                            <div class="row-fluid">
                                  <div class="span12">
                                            ...
                                            <button  onclick ="$('.statistic-table').tableExport({type:'excel',fileName:'测试桩(井)保护电位报表', excelstyles:['border-bottom', 'border-top', 'border-left', 'border-right']});" class="btn blue input-append"><i class="icon-add"></i>导出Excel</button>
                                            <button  onclick ="$('.statistic-table').tableExport({type:'word',fileName:'测试桩(井)保护电位报表'});" class="btn blue input-append"><i class="icon-add"></i>导出Word</button>
                                   </div>
...
<script>
    ...
    $(function () {
        ...
        handleDatePickers();
        TestPileStats.initForPotentialReport();
    })
    $("#searchMarker").click(function(){
        var year = $("#txt_begin_day").val();
        var ownerComp = $("#belong-comp").val();
        TestPileStats.initForPotentialReport(ownerComp,year)
    });
    $('.date-picker').datepicker({
        format: 'yyyy',
        weekStart: 1,
