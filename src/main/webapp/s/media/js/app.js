...
//        alert(height);
    ...
    var initBaidu = function () {
        if (jQuery("#container").size() === 1) {
            var height = _calculateFixedSidebarViewportHeight() - 5;
//            alert(height);
            jQuery("#container").attr('style', 'height:' + height + "px;");
        }
    }
...
//        return sidebarHeight;
        var sidebarHeight;
        if(document.body.clientHeight) {// chrome $(window).height() only added
            sidebarHeight = document.body.clientHeight;
        } else {

        }
        sidebarHeight = $(window).height();
        sidebarHeight = sidebarHeight - 20 - $('.header').height();
        return sidebarHeight;
    }
